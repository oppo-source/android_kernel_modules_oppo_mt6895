...
int cts_resume_device(struct cts_device *cts_dev)
{
    int ret = 0;
    int retries = 3;
    struct chipone_ts_data *cts_data = chipone_ts_data;//container_of(cts_dev, struct chipone_ts_data, cts_dev);
    ...
    if (retries < 0) {
        ...
        ret = cts_fw_update_in_probe(cts_data, &cts_data->tsdata->panel_data, false);
        if (ret) {
            cts_err("Update  firmware failed");
            goto err_set_program_mode;
        }
#endif
    }
...

    {
        ...
        if (r) {
            cts_err("Vendor config firmware failed %d(%s)",
                r, cts_strerror(r));
        }
#endif
    }
