...
#define RQ_LOCKING_TASK_LIMIT (6)
					...
					struct sched_entity **se, bool *repick)
{
	...
	if (orq->rq_picked_locking_cont >= RQ_LOCKING_TASK_LIMIT)
		goto pick_locking_failed;
...
pick_locking_failed:
	if (orq->rq_picked_locking_cont > 0)
		orq->rq_picked_locking_cont -= 1;
}
